...
PATHWAY_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("deepseek", re.compile(r"(?i)\bdeepseek\b|\bollama\b|\blocal.?open.?weights\b")),
    ("white_moon", re.compile(r"(?i)\bwhite\s*moon\b|\beast(?:ern)?\s*(?:white\s*)?moon\b")),
    ("discord", re.compile(r"(?i)\bdiscord\b|\bstatus.?notify\b")),
    ("kimi3", re.compile(r"(?i)\bkimi\b|\bmoonshot\b|\b3rd\s*leverage\b|\bthird\s*leverage\b")),
    ("central_control", re.compile(r"(?i)\bcentral.?control\b|\basuna.?point.?0\b")),
    ("stripe", re.compile(r"(?i)\bstripe\b|\bcheckout\s*session\b|\bbilling\b")),
...
def main() -> int:
    ...
    event = {
        ...
        "engine_routes": {
            ...
            "deepseek_asuna_point0": "python3 tools/deepseek_asuna_point0_unify.py write",
            "grok_xai_asuna_point0": "python3 tools/grok_xai_asuna_point0_unify.py write",
